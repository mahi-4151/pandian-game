...
export class Hud {
  ...
  #buildWeaponStrip() {
    // Each chip is a real button: tap it to equip that weapon directly.
    // The first chip (Vaal/sword) always puts the kathi back in the hand.
    this.el.weaponStrip.innerHTML = WEAPONS.map(
      (weapon) =>
        `<button type="button" class="weapon-chip" data-chip="${weapon.id}" data-weapon-chip="${weapon.id}" title="Equip ${weapon.label}">${weapon.name}</button>`,
    ).join('');
  ...
  showResult({ win, coins, gems, lives, points = 0, bonus = null, reward = null, wallet = null, level = 1, lastLevel = false }) {
    ...
    this.el.resultTitle.textContent = win ? 'Level Cleared!' : 'Level Not Cleared';
    this.el.resultSub.textContent = win
      ? `Level ${level} — ${this.levelTitle ?? 'The warlord'} is defeated. Madurai holds its throne.`
      : lives > 0
        ? `You have not completed the level and you have only ${lives} ${lives === 1 ? 'life' : 'lives'} left!`
        : 'You have not completed the level and you have no lives left. One heart refills every hour!';
    ...
    // Level Cleared shows the bonus breakdown + CONTINUE; Level Not Cleared
    // shows TRY AGAIN (only while hearts remain) so the player knows exactly
    // how many lives they have before going back in.
    ...
    // Win → CONTINUE + BACK TO MAP; Lose → TRY AGAIN (while lives remain) + BACK TO MAP.
    this.el.again.hidden = !win;
    this.el.retry.hidden = win || lives <= 0;
    const retryLabel = this.el.retry.querySelector('span');
    ...
    this.#heartsLossPopup(win ? null : lives);
  }

  #heartsLossPopup(remaining) {
    let overlay = document.getElementById('heartsLossPopup');
    ...
    if (!overlay) {
      ...
      overlay.id = 'heartsLossPopup';
      overlay.className = 'hearts-loss-overlay';
      overlay.innerHTML = '<div class="hearts-loss-box"><span data-hearts-loss-text></span></div>';
      overlay.addEventListener('click', () => overlay.classList.remove('show'));
      document.body.appendChild(overlay);
    }
    const text = overlay.querySelector('[data-hearts-loss-text]');
    if (text) text.textContent = `You only have ${remaining} hearts left now!`;
