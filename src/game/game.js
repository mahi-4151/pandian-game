...
import { AssetLoader, createBoxFallbackGltf } from '../engine/assets.js';
...
export class Game {
  constructor(canvas) {
    ...
    // Lives are the shared hearts pool (lives.js): whatever the map shows is
    // what the duel gets. One heart refills every hour.
    this.lives = window.PandyaLives ? PandyaLives.current().lives : COMBAT.lives;
    this.round = 1;
  ...
  static loadWallet() {
    if (window.PandyaWallet) return PandyaWallet.get();
    try {
    ...
    // Fresh players start the campaign with 100 coins and 50 pearls; the same
    // wallet is shown on every page (home, map, battle…).
    return { coins: 100, pearls: 50 };
  }

  /**
   * Real-time heart refill (lives.js). With zero lives left the duel is
   * blocked until the next heart is full — 1 hour per broken heart, so a
   * completely drained pool of 5 hearts takes 5 hours to fill again.
   * Returns remaining ms (0 = free to fight).
   */
  static getLockoutRemaining() {
    const shared = window.PandyaLives ? PandyaLives.current() : null;
    return shared && shared.lives <= 0 ? shared.msToNext : 0;
  }

  static isLocked() {
    const shared = window.PandyaLives ? PandyaLives.current() : null;
    return shared ? shared.lives <= 0 : false;
  }

  static loadUnlocked() {
    return window.PandyaProgress.unlocked();
  }

  /** Credits the hero for a level won, persists the wallet and unlocks the next. */
  rewardHero() {
    if (window.PandyaWallet) {
      this.wallet = PandyaWallet.add(COMBAT.reward.coins, COMBAT.reward.pearls);
    } else {
    ...
    const nextUnlocked = window.PandyaProgress.completeLevel(this.level);
  ...
  async boot() {
    ...
    let villainGltf;
    try {
      [heroGltf, villainGltf] = await Promise.all([
        this.loader.load(ASSETS.hero),
        this.loader.load(ASSETS.villain),
        rack.preload(),
      ]);
    } catch (error) {
      console.warn('[pandya] Character / weapon GLBs failed; using loader fallbacks.', error);
      [heroGltf, villainGltf] = await Promise.all([
        this.loader.load(ASSETS.hero),
        this.loader.load(ASSETS.villain),
      ]);
    ...
    const makeFighter = (gltf, options) => {
      try {
        return new Fighter({ gltf, ...options });
      } catch (error) {
        console.warn('[pandya] Fighter setup failed, using BoxGeometry fallback.', error);
        return new Fighter({ gltf: createBoxFallbackGltf(options.side === 'enemy' ? ASSETS.villain : ASSETS.hero), ...options });
      }
    };

    this.player = makeFighter(heroGltf, {
      rack,
      side: 'player',
    ...
    this.enemy = makeFighter(villainGltf, {
      rack,
      side: 'enemy',
    ...
    // Weapon strip: every chip is a direct-select button. The first chip is
    // the sword (Vaal/kathi) — after fighting with another weapon, tapping it
    // puts the sword back in the king's hand.
    for (const chip of this.hud.el.weaponStrip.querySelectorAll('[data-weapon-chip]')) {
      chip.addEventListener('click', () => this.equipWeaponById(chip.dataset.weaponChip));
    }

    try {
      await this.arena.buildSkyline();
    } catch (error) {
      console.warn('[pandya] Skyline skipped.', error);
    }
    try {
      await this.arena.buildBackdrop();
    } catch (error) {
      console.warn('[pandya] Backdrop skipped.', error);
    }
    ...
    this.hud.el.retry.addEventListener('click', () => this.retryAfterDefeat());
    ...
    // Honour the heart refill: with every heart broken, the duel stays
    // blocked until the next heart is full (1 hour per heart).
    if (Game.isLocked()) {
  ...
  continueLevel() {
    if (this.level < 9 && this.level + 1 <= Game.loadUnlocked()) {
      window.location.href = `battle.html?level=${this.level + 1}`;
    } else {
      // Milestone character selection (or final clear) returns to the map.
      window.location.href = 'map.html';
    }
  }

  /**
   * TRY AGAIN after the "Level Not Cleared" page. The level continues where
   * it stopped — the enemy keeps the wounds he already took — but a new
   * round only starts while the player still has a heart left.
   */
  retryAfterDefeat() {
    const shared = window.PandyaLives ? PandyaLives.current() : { lives: COMBAT.lives, msToNext: 0 };
    if (shared.lives <= 0) {
      // No hearts left → show the real-time refill countdown instead.
      this.hud.showLocked(shared.msToNext);
      return;
    }
    this.hud.hideResult();
  ...
  playerHeal() {
    if (this.cooldowns.heal > 0 || this.player.busy || this.player.blocking) return;
  ...
  cycleWeapon() {
    if (this.cooldowns.weapon > 0 || this.player.busy || this.player.blocking) return;
  ...
  /**
   * Direct weapon select from the weapon strip. The king always starts the
   * duel with the sword (Vaal/kathi); after switching to another weapon,
   * tapping the SWORD chip puts the kathi straight back into his hand.
   */
  equipWeaponById(id) {
    if (this.state !== 'fighting' || this.player.dead || this.player.busy || this.player.blocking) return;
    if (this.cooldowns.weapon > 0) return;
    if (this.player.weapon.id === id) {
      this.hud.announce(this.player.weapon.name);
      return;
    }
    this.cooldowns.weapon = 0.28;
    const weapon = this.player.setWeaponById(id);
    if (!weapon || weapon.id !== id) return;
    this.hud.setWeapon(weapon);
    this.sfx.swap();
    this.hud.announce(weapon.name);
    this.effects.burst(this.player.handPoint(weapon.hand, new THREE.Vector3()), {
  ...
  onDefeat(fighter) {
    ...
    // Player died — break one heart in the shared lives pool (1 hour refill).
    // A win never reaches this branch. Hearts never go below 0.
    this.lives = Math.max(0, this.lives - 1);
    window.PandyaLives?.lose();
    this.hud.setLives(this.lives);
    this.controls.setEnabled(false);
    this.player.setMove(0);
    this.state = 'over';
    ...
    // Level not cleared → the same kind of page as "Level Cleared" appears,
    // telling the player exactly how many lives (hearts) they have left.
    if (this.lives <= 0) {
