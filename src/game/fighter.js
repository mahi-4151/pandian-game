    ...
    await Promise.all(
      defs.map(async (def) => {
        try {
          ...
          if (gltf?.scene) this.sources.set(def.file, gltf.scene);
        } catch (error) {
          console.warn('[pandya] Weapon GLB failed (BoxGeometry fallback already applied if possible):', def.file, error);
        }
      }),
    );
  }
...
export class Fighter {
  constructor({ gltf, rack, side, height, startX, maxHp }) {
    ...
    if (boneBox.isEmpty()) {
      ...
      if (!meshBox.isEmpty()) {
        const meshHeight = Math.max(0.001, meshBox.max.y - meshBox.min.y);
        const scale = height / meshHeight;
        model.scale.multiplyScalar(scale);
        model.position.y = -meshBox.min.y * scale;
      }
    } else {
