...
(function () {
  ...
  let channel = null;
  try {
    channel = new BroadcastChannel('pandya.wallet');
    channel.onmessage = (event) => {
      if (event.data && Number.isFinite(event.data.coins) && Number.isFinite(event.data.pearls)) {
        wallet = { coins: event.data.coins, pearls: event.data.pearls };
        render();
      }
    };
  } catch (e) {
  ...
  function write(w) {
    ...
    try {
      window.dispatchEvent(new CustomEvent('pandya-wallet-change', { detail: { ...w } }));
    } catch (e) { /* ignore */ }
    try {
